  ...
  async fetch(request, env) {
    ...
    async function onMessage(message) {
      ...
      if (text === '/start') {
        ...
        await sendMessageToUser(chatId, `👋 *欢迎使用🍀千寻的传话筒*\n\n` +
              `📝 请使用礼貌用语进行对话\n` +
              `⏱ 管理员看到消息会及时回复\n\n` +
    ...
    async function getVerificationSuccessMessage() {
      ...
      const response = await fetch('https://raw.githubusercontent.com/QianXun28/TGChatBot/refs/heads/master/CFTeleTrans/start.md');
